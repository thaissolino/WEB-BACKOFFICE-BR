// src/components/FornecedoresTab.tsx
            ...
            {fornecedores.length > 0 ? (
              fornecedores.map((f) => (
                <tr key={f.id} className="hover:bg-gray-50 text-sm">
                  <td className="py-2 px-4 border text-center font-medium">{f.nome.toUpperCase()}</td>
                  <td className="py-2 px-4 border text-center">{f.taxa}</td>
                  <td className={`py-2 px-4 border text-center font-semibold ${f.saldo >= 0 ? "text-green-600" : "text-red-600"}`}>
