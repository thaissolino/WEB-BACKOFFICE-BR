...
export const CambioForm = ({ onRegistrarCompra, onAlocarDolar }: CambioFormProps) => {
  ...
  const [compraQuantidade, setCompraQuantidade] = useState('');
  const [compraTaxa, setCompraTaxa] = useState('');
  
  const [alocacaoData, setAlocacaoData] = useState(new Date().toISOString().split('T')[0]);
  const [alocacaoValor, setAlocacaoValor] = useState('');
  ...
  const handleAlocarDolar = () => {
    ...
    setCustoMedioAplicado(parseFloat(compraTaxa) || 0);
